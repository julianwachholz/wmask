...
(function (w) {
    ...
    Mask.Mask.prototype.getChunks = function (gridSize) {
        ...
        xs = Math.floor(this.selection.w / gridSize);
        ys = Math.floor(this.selection.h / gridSize);
        chunks = [];
    ...
    Mask.Mask.prototype.getImageData = function (gridSize) {
        var width, height, halfGrid;

        if (!!gridSize) {
            halfGrid = gridSize / 2;
            width = this.selection.w % gridSize;
            this.selection.w += width > halfGrid ? gridSize - width : -width;
        }
