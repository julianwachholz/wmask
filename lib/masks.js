...
(function (w, d) {
    ...
    Mask.Meko = function (direction) {
         ...
         */
        return function (callback) {
            var chunks, resultImage;
            chunks = this.getChunks(Mask.GRID_SIZE_DOUBLE);

            resultImage = this.putChunks(chunks, Mask.GRID_SIZE_DOUBLE);
            callback(resultImage,
                resultImage.width - (resultImage.width % Mask.GRID_SIZE_DOUBLE),
                resultImage.height - (resultImage.height % Mask.GRID_SIZE_DOUBLE));
        };
    };
