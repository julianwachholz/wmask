...
(function (w, d) {
    ...
    Mask.CP = function () {
        ...
        /**
         * @type {RegExp} Matching passcodes for CP masks.
         */
        CP.CODE_RE = new RegExp('^[a-z ]{1,' + Mask.CP_CODE_MAX_LENGTH + '}$', 'i');
         ...
         * @return {String}
         */
        CP.prototype.getCode = function () {
            var input;

            do {
                input = prompt('CP code');
            } while (input !== null && !CP.CODE_RE.test(input));
        ...
        return function (callback) {
            ...
            for (y = 0; y < cp.height; y += 1) {
                for (x = 0; x < cp.width; x += 1) {
                    ...
                    // TODO rotate chunk if rotate is true
                    // TODO swap red and green channels.
                }
            }
    ...
    Mask.Actions = {
        ...
        'cp': Mask.Chain([Mask.Xor(Mask.NEG), Mask.CP()]),
    };
