...
/*global Point: true, Rectangle: true,
         requestAnimationFrame: true, FileReader: true
*/
...
(function (w, d) {
    ...
    WMask.prototype.drawSelection = function () {
        ...
        if (this.selection.hasArea()) {
            var clipboard = document.getElementById('clipboard'),
                clip = clipboard.getContext('2d'),
                s = this.selection;

            clipboard.width = this.selection.w;
            clipboard.height = this.selection.h;
            clip.drawImage(this.image, s.x, s.y, s.w, s.h, 0, 0, s.w, s.h);
            document.getElementById('l').innerHTML = 'Selection size: ' + s.w + '×' + s.h;
        }
    };
