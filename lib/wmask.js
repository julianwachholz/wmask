...
(function (w, d) {
    ...
    /**
     * Prepare our web worker.
     */
    WMask.prototype.prepareWorker = function () {
        if (this.worker !== null) {
            return;
        }
        this.worker = new Worker('lib/mask.worker.js');
        this.worker.addEventListener('message', function (event) {
            if (event.data.status === 'progress') {
                this.showProgress('Masking, please wait...', event.data.progress, event.data.frame);
            }
