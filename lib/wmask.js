...
/*global Point: true, Rectangle: true, Mask: true, ga: true,
         requestAnimationFrame: true, Event: true, MouseEvent: true, FileReader: true
*/
...
(function (w, d) {
    ...
    WMask.prototype.initMenu = function () {
        var maskButtons = [], mask;
        id('menu').style.display = 'block';

        for (mask in Mask.Actions) {
            if (Mask.Actions.hasOwnProperty(mask)) {
                id('mask-' + mask).addEventListener('click', this.applyMask.bind(this, Mask.Actions[mask], mask));
                maskButtons.push(id('mask-' + mask));
            }
        }

        ['save', 'undo', 'redo', 'zoomin', 'zoomout'].forEach(function (action) {
            var actionMethod = action + 'Action';
            id('action-' + action).addEventListener('click', this[actionMethod].bind(this));
        }.bind(this));
     ...
     */
    WMask.prototype.applyMask = function (maskFunc, maskName) {
     ...
     */
    WMask.prototype.undoAction = function () {
        console.warn('undo not implemented yet.');
    };

    /**
     * Repeat the last action we just reverted.
     */
    WMask.prototype.redoAction = function () {
        console.warn('redo not implemented yet.');
    };

    /**
     * Zoom into the viewport of the image.
     */
    WMask.prototype.zoominAction = function () {
        console.warn('zoomin not implemented yet.');
    };

    /**
     * And zoom back out again.
     *
     * TODO add Ctrl+0 shortcut to reset zoom.
     */
    WMask.prototype.zoomoutAction = function () {
        console.warn('zoomout not implemented yet.');
    };
