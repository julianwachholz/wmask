...
/*global Point: true, Rectangle: true, Mask: true, ga: true,
         requestAnimationFrame: true, Event: true, MouseEvent: true, CustomEvent: true, FileReader: true
*/
...
(function (w, d) {
     ...
     */
    WMask.RE_FILENAME = /^.*?([^\/]*?)\.(jpe?g|png|gif|bmp)$/i;
    ...
    WMask.prototype.acceptImage = function () {
        var img, c, drop, click, readFile, gaLoadFunction, showError, setFilename;
        ...
        click = function () {
            ...
            fileInput.addEventListener('change', function () {
                if (this.files.length < 1 || this.files[0].type.indexOf('image') === -1) {
    ...
    WMask.prototype.applyMask = function (maskName) {
        var canvas = document.createElement('canvas'), selection, putImageData, drawContext, cpCode;

        selection = this.selection;
        putImageData = function (imageData, w, h) {
            if (w && h) {
                this.imageCtx.putImageData(imageData, selection.x, selection.y, 0, 0, w, h);
            } else {
                this.imageCtx.putImageData(imageData, selection.x, selection.y);
            }
        }.bind(this);
        ...
        drawContext = function () {
            ...
            context.drawImage(this.imageCanvas,
        ...
        this.history.add({
            ...
            forward: function () {
                ...
                mask = new Mask.Mask(func, selection, drawContext());
                mask.applyMask(putImageData);
            },
            backward: function () {
                var mask, func;
        ...
        ga('send', 'event', 'image', 'mask', maskName);
    };
    ...
    WMask.prototype.saveAction = function () {
        var link, image = this.imageCanvas.toDataURL();
     ...
     *
     * @param {Number} zoom Relative zoom level to apply. Zero resets soom.
     */
    WMask.prototype.zoomAction = function (zoom) {
        console.warn('zoom not implemented yet.', zoom);
        /*
        TODO disable image smoothing for enlarged canvas.
        TODO enable image smoothing for reduced canvas.
    ...
    WMaskHistory.prototype.forward = function () {
        this.position += 1;
        this.entries[this.position].forward();
    ...
    WMaskHistory.prototype.backward = function () {
        this.entries[this.position].backward();
        this.position -= 1;
