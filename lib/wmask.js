...
/*global Point: true, Rectangle: true, Mask: true, ga: true, Whammy: true,
         requestAnimationFrame: true, cancelAnimationFrame: true, URL: true, Event: true, MouseEvent: true, CustomEvent: true, FileReader: true
*/
...
(function (w, d) {
     ...
     * @type {Number}
     */
    WMask.prototype.frameRate = 1 / 29.97;
    ...
    WMask.prototype.showStartScreen = function () {
        ...
        this.ctx.fillText('Drag image here or click to select.', this.width / 2, this.height / 2);
    };
    ...
    WMask.prototype.acceptMedia = function () {
        ...
        loadVideo = function (src) {
            var v = d.createElement('video'), c;

            c = this.canvas;
            v.addEventListener('play', function play() {
                c.removeEventListener('click', click);
                c.removeEventListener('drop', drop);
                c.addEventListener('drop', function (event) {
                    event.preventDefault();
        ...
        drop = function (event) {
            var type, tx = event.dataTransfer;
            event.preventDefault();
    ...
    WMask.prototype.captureFrames = function (media) {
        ...
        setTimeout(function capture() {
            ...
            if (!media.seeking) {
                ...
                currentFrame += 1;
                this.showProgress('Capturing video frames...', currentFrame / totalFrames);
            }
    ...
    WMask.prototype.applyMask = function (maskName) {
        ...
        this.history.add({
            mask: maskName,
            forward: function (context, callback) {
                var mask, func;

                func = Mask.Actions[maskName];
                if (!!cpCode) {
                    func = Mask.Actions[maskName](cpCode);
                }
                mask = new Mask.Mask(func, selection.w, selection.h, context);
                mask.applyMask(callback);
            },
            backward: function (context, callback) {
                var mask, func;

                func = Mask.ReverseActions[maskName];
                if (!!cpCode) {
                    func = Mask.ReverseActions[maskName](cpCode);
                }
                mask = new Mask.Mask(func, selection.w, selection.h, context);
                mask.applyMask(callback);
            },
     ...
     */
    WMask.prototype.processFrames = function (maskFunc) {
        var selection, drawContext, newFrames, totalFrames;
        selection = this.selection;

        drawContext = function (frame) {
            ...
            tempCanvas = d.createElement('canvas');
        ...
        setTimeout(function renderFrame(frameIndex) {
            var completeFrame;
            this.showProgress('Masking, please wait...', frameIndex / totalFrames);
            completeFrame = this.frames[frameIndex];

            maskFunc(drawContext(completeFrame), function (imageData, w, h) {
                this.frameCtx.putImageData(completeFrame, 0, 0);
                if (w && h) {
                    this.frameCtx.putImageData(imageData, selection.x, selection.y, 0, 0, w, h);
                } else {
                    this.frameCtx.putImageData(imageData, selection.x, selection.y);
                }
                newFrames[frameIndex] = this.frameCtx.getImageData(0, 0, this.width, this.height);

                if (frameIndex + 1 === totalFrames) {
                    this.frames = newFrames;
                    this.draw();
                } else {
                    setTimeout(renderFrame.bind(this, frameIndex + 1));
                }
            }.bind(this));
        }.bind(this, 0));
    };
    ...
    WMask.prototype.saveVideo = function () {
        ...
        encoder = new Whammy.Video(1 / this.frameRate);
        ...
        cancelAnimationFrame(this.animationID);
        this.showProgress('Compiling video, please be patient...', -1);
        setTimeout(function () {
            this.frames.forEach(function (frame, i) {
                ...
                if (i + 1 === frameCount) {
                    ...
                    link.href = URL.createObjectURL(encoder.compile());
                    link.click();
                    this.draw();
                }
            }.bind(this));
    ...
    WMask.prototype.historyAction = function (direction) {
        if (direction === 'undo') {
            this.processFrames(this.history.backward());
        }
        if (direction === 'redo') {
            this.processFrames(this.history.forward());
        }
    ...
    WMask.prototype.zoomAction = function (zoom) {
        ...
        this.setCanvasSize(this.image.width * this.zoomLevel, this.image.height * this.zoomLevel);
    };
    ...
    WMask.prototype.draw = function () {
        var frameTiming, frameCount, currentFrame, isVideo, animationFrame;
    ...
    WMaskHistory.prototype.forward = function () {
        ...
        return this.entries[this.position].forward;
    };
    ...
    WMaskHistory.prototype.backward = function () {
        var backward = this.entries[this.position].backward;
        this.position -= 1;
